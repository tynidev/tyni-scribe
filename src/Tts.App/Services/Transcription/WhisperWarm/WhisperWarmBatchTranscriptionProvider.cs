...
public sealed class WhisperWarmBatchTranscriptionProvider : IBatchTranscriptionProvider
{
    ...
    public TranscriptionProviderMetadata Metadata { get; } = new(
        ...
        "Keeps a local whisper.cpp server warm between recordings.");

    public IReadOnlyList<ProviderSettingDescriptor> SettingDescriptors => WhisperWarmProviderSettings.Descriptors;
    ...
    public async Task<BatchTranscriptionResult> TranscribeAsync(BatchTranscriptionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var settings = WhisperWarmProviderSettings.Parse(request.Settings);
        ...
        var serverExecutablePath = WhisperCppRuntimePaths.ResolveServerExecutablePath(settings);
